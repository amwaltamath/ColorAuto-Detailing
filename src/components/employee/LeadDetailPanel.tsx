    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="p-4 bg-gray-50 border-b border-gray-200 flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-500 text-sm">{getSourceLabel(lead.source)}{lead.utm_campaign ? ` · ${lead.utm_campaign}` : ''}</p>
        </div>
        <button onClick={onClose} className="text-gray-400 hover:text-gray-700 text-xl leading-none ml-4">&times;</button>
      </div>

      <div className="p-4 space-y-4">
        {/* Status Pipeline */}
        <div>
          <label className="text-xs font-medium text-gray-500 uppercase mb-2 block">Pipeline Status</label>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {lead.email && (
            <a href={`mailto:${lead.email}`} className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
              <span className="text-blue-500">✉️</span>
              <span className="text-sm text-gray-900 truncate">{lead.email}</span>
            </a>
          )}
          {lead.phone && (
            <a href={`tel:${lead.phone}`} className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <p className="text-gray-900">{lead.vehicle_info || '—'}</p>
          </div>
          <div>
            <span className="text-gray-500 text-xs">Est. Value</span>
            <p className="text-gray-900">{lead.estimated_value ? `$${Number(lead.estimated_value).toLocaleString()}` : '—'}</p>
          </div>
          <div>
            <span className="text-gray-500 text-xs">Created</span>
            <p className="text-gray-900 text-xs">{formatDate(lead.created_at)}</p>
          </div>
        </div>
        ...
        {lead.message && (
          <div>
            ...
            <p className="text-gray-700 text-sm bg-gray-50 p-3 rounded-lg mt-1">{lead.message}</p>
          </div>
        )}
        ...
        {(lead.utm_source || lead.utm_medium || lead.utm_campaign) && (
          <div>
            <span className="text-gray-500 text-xs uppercase font-medium">Ad Tracking</span>
            <div className="flex flex-wrap gap-1.5 mt-1">
              {lead.utm_source && <span className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded">src: {lead.utm_source}</span>}
              {lead.utm_medium && <span className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded">med: {lead.utm_medium}</span>}
              {lead.utm_campaign && <span className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded">camp: {lead.utm_campaign}</span>}
            </div>
            {lead.landing_page && <p className="text-xs text-gray-400 mt-1 truncate">Landing: {lead.landing_page}</p>}
          </div>
        )}

        {/* Notes / Follow-ups */}
        <div>
          <h4 className="text-sm font-medium text-gray-700 mb-2">Notes & Follow-ups</h4>
          ...
          {loadingNotes ? (
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {notes.map(n => (
                <div key={n.id} className="bg-gray-50 p-2.5 rounded-lg border border-gray-200">
        ...
        <div className="flex gap-2 pt-2 border-t border-gray-200">
          ...
          <button
            ...
            className="px-3 py-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition text-sm border border-red-200"
          >
            🗑️ Delete
          </button>
        </div>
